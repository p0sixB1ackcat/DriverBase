...
//lock
ERESOURCE g_RuleLock;
//保存R3下发的规则
LIST_ENTRY g_RuleEntry;
...
VOID RemoveAllRuleListEntry(PLIST_ENTRY header);
...
NTSTATUS ScannerNotifyMessage(PVOID PortCookie
	, PVOID InputBuffer
...
BOOLEAN 
ScannerpCheckExtensionNew(
	PUNICODE_STRING Extesion
);

VOID
ConvertRuleListEntry(
	PLIST_ENTRY header
);
...
--*/
{
	...
	if (NT_SUCCESS(status)) {
		...
		status = FltCreateCommunicationPort(ScannerData.Filter,
			...
			ScannerPortConnect,
			ScannerPortDisconnect,
			ScannerNotifyMessage,//作业，补充
			1);

		ExInitializeResourceLite(&g_RuleLock);

		InitializeListHead(&g_RuleEntry);
...
--*/
{
	...
	RemoveAllRuleListEntry(&g_RuleEntry);

	DbgPrint("Start Check RuleList is Empty!\n");
	ConvertRuleListEntry(&g_RuleEntry);
}

NTSTATUS AddRuleToListEntry(PVOID inputBuffer, ULONG inputBufferLength)
{
	WCHAR *p = (WCHAR *)inputBuffer;
	ULONG index = 0x00;
	ULONG i = 0x00;
	ULONG dwCurLen = 0x00;
	SCANNER_RULE *rule = NULL;
	ULONG dwMemLength = 0x00;
	...
	while (index < inputBufferLength)
	{
		if (*(p + index) == ',')
		{
			dwCurLen = sizeof(WCHAR) * (index - i);
			dwMemLength = sizeof(SCANNER_RULE) + dwCurLen;

			rule = (SCANNER_RULE *)ExAllocatePoolWithTag(PagedPool
			,dwMemLength + 2 * sizeof(WCHAR)
			,'ELUR');
			...
			RtlZeroMemory(rule, dwMemLength);

			DbgPrint("rule->buffer address is 0x%x!\n", (ULONG)rule->buffer);

			memcpy(rule->buffer,(WCHAR *)inputBuffer + i, dwCurLen);
			rule->buffer[dwCurLen] = '\0';

			LockRule(&g_RuleLock);

			InsertTailList(&g_RuleEntry, &rule->listEntry);

			UnlockRule(&g_RuleLock);
			i = index + 1;
		}
		++index;
	}
	...
	{
		dwCurLen = sizeof(SCANNER_RULE) + sizeof(WCHAR) * (inputBufferLength - i);
		rule = (PSCANNER_RULE)ExAllocatePoolWithTag(PagedPool, dwCurLen + sizeof(WCHAR), 'ELUR');

		if (!rule)
		{
			return STATUS_MEMORY_NOT_ALLOCATED;
		}
		RtlZeroMemory(rule,dwCurLen);
		
		memcpy(rule->buffer, (WCHAR *)inputBuffer + i, sizeof(WCHAR) * (inputBufferLength - i));
		rule->buffer[inputBufferLength - i] = '\0';
		LockRule(&g_RuleLock);
		InsertTailList(&g_RuleEntry, &rule->listEntry);
		UnlockRule(&g_RuleLock);
...
VOID RemoveAllRuleListEntry(PLIST_ENTRY header)
{
	PLIST_ENTRY lpCurList = NULL;
	PLIST_ENTRY lpRmList = NULL;
	PSCANNER_RULE lpRule = NULL;

	if (!header)
		return;
	lpCurList = header->Flink;
	while (lpCurList != header)
	{
		lpRmList = lpCurList;
		
		lpRule = CONTAINING_RECORD(lpCurList,SCANNER_RULE,listEntry);
		lpCurList = lpCurList->Flink;
		RemoveEntryList(lpRmList);

		//BAD_POOL_HEADER:lpRule - 8
		ExFreePoolWithTag(lpRule, 'ELUR');
		
		lpRule = NULL;
	}
}

VOID ConvertRuleListEntry(PLIST_ENTRY header)
{
	PLIST_ENTRY lpCurList = NULL;
	PSCANNER_RULE lpRule = NULL;
	...
	do
	{
		...
		if (!lpRule)
		{
			DbgPrint("Containing_Record faild!\n");
			UnlockRule(&g_RuleLock);
			return;
		}

		DbgPrint("lpRule->buffer is %ws!\n", lpRule->buffer);
		//lpRule = NULL;
		lpCurList = lpCurList->Flink;
...
BOOLEAN ScannerpCheckExtensionNew(PUNICODE_STRING Extesion)
{
	UNICODE_STRING uCurRule = {0x00};
	PLIST_ENTRY lpRuleList;
	PSCANNER_RULE lpRule = NULL;
	...
	lpRuleList = g_RuleEntry.Flink;
	while (lpRuleList != &g_RuleEntry)
	{

		lpRule = CONTAINING_RECORD(lpRuleList,SCANNER_RULE,listEntry);
		lpRuleList = lpRuleList->Flink;
		if (!lpRule)
		{
			continue;
		}
		RtlInitUnicodeString(&uCurRule, lpRule->buffer);
		uCurRule.Length = sizeof(WCHAR) * wcslen(lpRule->buffer);
		uCurRule.MaximumLength = PBC_MAX_PATH;
		if (RtlCompareUnicodeString(Extesion, &uCurRule,TRUE) == 0)
		{
			UnlockRule(&g_RuleLock);
			return TRUE;
		}
...
--*/
{
	...
	scanFile = ScannerpCheckExtensionNew(&nameInfo->Extension);
