...
NTSTATUS __stdcall PBC_PspTerminateProcess(PEPROCESS process,NTSTATUS ExitStatus)
{
	DbgPrint("enter PBC_PspTerminateProcess!\n");
	return 1;
}

_declspec (naked)T_PBC_PspTerminateProcess(PEPROCESS process, NTSTATUS ExitStatus)
{
	NTSTATUS ioStatus;
	__asm
	{
...
VOID DriverUnload(PDRIVER_OBJECT pDriObj)
{
	...
	DbgPrint("goodbye PBCInLinkHook!\n");
}
...
VOID InlineHookPspTerminateProcess()
{
	...
	*lpBuffer = 0xE9; 
	...
	uJmpOffset = (ULONG)T_PBC_PspTerminateProcess - ((ULONG)PspTerminateProcess + uTotalLen);
	RtlCopyMemory(lpBuffer + 1,&uJmpOffset,4);

	ioStatus = WriteKernelMemory(PspTerminateProcess, lpBuffer, uTotalLen);
	DbgPrint("T_PBCPspTerminateProcess address is 0x%x\n", (ULONG)T_PBC_PspTerminateProcess);
...
NTSTATUS DriverEntry(__in PDRIVER_OBJECT pDriObj,__in PUNICODE_STRING pDriPath)
{
	...
	PspTerminateProcess = (PSPTERMINATEPROCESS)PBCMultiThreadSearchFunAddr(L"PspTerminateProcess");
