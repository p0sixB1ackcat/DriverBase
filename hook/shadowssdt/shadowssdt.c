...
#define DeviceName L"\\device\\PBCShadowSSDT"
#define SymbolicName L"\\dosDevices\\PBCShadowSSDT"
...
BOOL PBC_NtGdiBitBlt(IN HDC hdcDes
					   ,IN int xDes
					   ,IN int yDes
					   ,IN int cxDes
					   ,IN int cyDes
...
BOOL PBC_NtGdiStretChBlt(IN HDC hdcDes
						 ,IN int xDes
						 ,IN int yDes
						 ,IN int cxDes
						 ,IN int cyDes
...
NTSTATUS PBC_NtUserHwndQueryRedirectoryInfo(ULONG param1, ULONG param2, ULONG param3, ULONG param4);
...
BOOLEAN PBCSleep(ULONG TimeInterval)
{
...
VOID DriverUnload(PDRIVER_OBJECT pDriObj)
{
	...
	DbgPrint("PBCShadowSSDT : goodbye!\n");
...
VOID PBCHook(VOID)
{
	...
	__try
	{
		...
		OldNtGdiBitBlt = (NTGDIBITBLT)InterlockedExchange((PULONG)&g_lpKeServiceTableBaseShadow->ServiceTableBase[13],(ULONG)PBC_NtGdiBitBlt);
		OldNtGdiStretChBlt = (NTGDISTRETCHBLT)InterlockedExchange((PULONG)&g_lpKeServiceTableBaseShadow->ServiceTableBase[292]
		,(ULONG)PBC_NtGdiStretChBlt);
...
NTSTATUS DriverEntry(PDRIVER_OBJECT pDriObj,PUNICODE_STRING pDriPath)
{
	...
	DbgPrint("start PBC_ShadowSSDT_Hook!\n");
	PBCHook();
