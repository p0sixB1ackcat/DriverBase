...
BOOL  HandleData(OP_INFO *pOpInfoData)
{
	...
	CPopupDlg dlg;
	CString strMsg;
	strMsg.Format(_T("拦截到驱动加载进程：%s"),pOpInfoData->m_ProcessName);
	dlg.SetProcess(strMsg.GetBuffer());
	strMsg.ReleaseBuffer();
	strMsg.Format(_T("目标模块%s"),pOpInfoData->m_DestName);
	dlg.SetDetail(strMsg.GetBuffer());
	strMsg.ReleaseBuffer();
	dlg.DoModal();
